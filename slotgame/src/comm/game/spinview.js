...
function CSpinView(mainView, game)
{
	...
	this.init = function() {
		...
		// 	tweenLoop = game.add.tween(reel).to( { y: y - 150 }, 700, Phaser.Easing.Linear.None, false, 0, 0);
  //   		// tweenLoop.onRepeat.add(this.onTweenLoop, this);
		// 	tweenLoop.onComplete.add(this.onComplete2, this);
		// 	tweens_Loop.push(tweenLoop);
		// }
	}
	...
	this.initReel = function() {
		for(var i = 0; i < g_confReel.length; i++) {
			...
			var wheel = new CReel(game, reelX, reelY, i, this);
			wheel.initImage();
 ...
 //   		tweens_Loop[reel.index].start();
	// }
	// this.onTweenLoop = function(reel) {
	// 	if(game_status != GAME_STAUS.WAITING) {
	// 		tweens_Loop[reel.index].stop(true);
	// 		this.onComplete2(reel);
	// 	}
	// 	else {
	// 		reel.y = initY[reel.index];
	// 	}
	// }

	// this.onComplete1 = function(reel) {
	// 	reel.y = initY[reel.index];
	// 	var y = initY2[reel.index];
	// 	tweenLoop.start();
	// }
	...
	this.onSpinResult = function(result, elapse) {
		if(result) {
			...
			//}
		// 	if(result.success)
		// 		game.time.events.add(Phaser.Timer.SECOND * 3, this.onSpinSuccess, this);
		}
	}
