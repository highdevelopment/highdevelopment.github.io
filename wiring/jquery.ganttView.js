...
(function(jQuery) {
    ...
    var Chart = function(div, opts) {
        ...
        function addHzHeader(div, dates, cellWidth) {
            ...
            for(var y in dates) {
                for(var m in dates[y]) {
                    for(var d in dates[y][m]) {
                        // Day
                        var dayNow = dates[y][m][d];
                        var width = cellWidth * 24;
                        totalW += width;
                        monthsDiv.append(jQuery("<div>", {
                            "class": "ganttview-hzheader-month",
                            "css": {"width": (width - 1) + "px"}
                        }).append(dayNames[dayNow.getDay()] + ", " + monthNames[m] + ' ' + dayNow.getDate() + ", " + y));
                        // Hour
            ...
            headerDiv.append(daysDiv).append(monthsDiv);
            div.append(headerDiv);
        }
        ...
        function addBlocks(div, data, cellWidth, start) {
            ...
            for(var i = 0; i < data.length; i++) {
                for(var j = 0; j < data[i].series.length; j++) {
                    ...
                    var size = DateUtils.hoursBetween(series.start, series.end);
                    var offset = DateUtils.hoursBetween(start, series.start);
                    var block = jQuery("<div>", {
                        "class": "ganttview-block",
                        "title": series.name + ", " + size + " hours, offset: " + offset,
                        "css": {
                            "width": ((size * cellWidth) - 9) + "px",
                            "left": (start.getHours() * cellWidth + offset * cellWidth + 3) + "px",
                            "position": "absolute"
                        }
                    });
